label gpquiz1():
    ...
    while num_questions > 0:
        $ selected_question = next_rnd_in_list(question_masterlist2)
...
label gpquiz2():
    ...
    while num_questions > 0:
        $ selected_question = next_rnd_in_list(question_masterlist3)
        call expression selected_question
...
label bround2:
    ...
    if bonus2.lower() in ["animals"]:
        hide screen bonus2
        ct "Wonderful [player_name]!"
        ct "You got a additional heart that would aid you in your journey. ^_^"
        $ max_lives += 1
        $ lives += 1
        return "pass"
